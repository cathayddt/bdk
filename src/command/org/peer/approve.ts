...
export const builder = (yargs: Argv<OptType>) => {
  return yargs
    .example('bdk org peer approve --interactive', 'Cathay BDK 互動式問答')
    .example('bdk org peer approve --channel-name test', '對channel test 的更動資訊簽章')
...
export const handler = async (argv: Arguments<OptType>) => {
  ...
  await peer.approve({ channelName })
}
