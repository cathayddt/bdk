...
export const builder = (yargs: Argv<OptType>) => {
  return yargs
    .example('bdk fabric network create --interactive', 'Cathay BDK 互動式問答')
    .example('bdk fabric network create --file ~/.bdk/network-create.json --create-full', '使用在路徑下 json 檔案中的參數，產生 Blockchain network 所需的相關設定檔案')
    .example('bdk fabric network create --file ~/.bdk/network-create.json --cryptogen', '使用在路徑下 json 檔案中的參數和 cryptogen，產生憑證和私鑰')
    .example('bdk fabric network create --file ~/.bdk/network-create.json --genesis', '使用在路徑下 json 檔案中的參數，產生創始區塊')
    .example('bdk fabric network create --file ~/.bdk/network-create.json --connection-profile', '使用在路徑下 json 檔案中的參數，產生 Peer 連接設定檔案')
    .example('bdk fabric network create --file ~/.bdk/network-create.json --docker-compose', '使用在路徑下 json 檔案中的參數，產生 Peer/Orderer docker-compose 檔案')
