...
export const builder = (yargs: Argv<OptType>) => {
  return yargs
    .example('bdk fabric chaincode approve --interactive', 'Cathay BDK 互動式問答')
    .example('bdk fabric chaincode approve --channel-id test --chaincode-label fabcar_1 --init-required', '使用 discover 自動選擇 orderer 同意名稱為 test 的 channel 中標籤為 fabcar_1 的 Chaincode，此 Chaincode 需要初始化')
    .example('bdk fabric chaincode approve --channel-id test --chaincode-label fabcar_1 --init-required --orderer orderer0.example.com:7050', '使用 orderer0.example.com:7050 同意名稱為 test 的 channel 中標籤為 fabcar_1 的 Chaincode，此 Chaincode 需要初始化')
