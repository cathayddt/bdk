...
// export default class Validator extends AbstractService {
//   /**
//    * @description 啟動 orderer org 的機器
//    */
//   public async up (dto: OrdererUpType): Promise<InfraRunnerResultType> {
//     logger.debug(`Validator up: ${dto.ordererHostname}`)
...
//     logger.debug(`Validator down: ${dto.ordererHostname}`)
...
//     logger.debug('Validator create cryptogen')
...
//       logger.debug(`Validator create configtx: ${ordererOrg.name}`)
...
//       logger.debug(`Validator create copyTLSCa: ${ordererOrg.name}`)
...
//       logger.debug(`Validator create docker-compose: ${ordererOrg.name}`)
