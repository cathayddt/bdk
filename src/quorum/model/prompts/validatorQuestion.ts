...
import { NetworkCreateValidatorType, NetworkValidatorPortType } from '../type/network.type'

export const validatorQuestions = async (count: number): Promise<NetworkCreateValidatorType> => {
  // const networkCreatePeerOrg: NetworkCreatePeerOrg = {}
  const { name } = await prompts([
    {
      type: 'text',
      name: 'name',
      message: `What is validator org ${count} name?`,
  ...
  const ports: NetworkValidatorPortType[] = []
