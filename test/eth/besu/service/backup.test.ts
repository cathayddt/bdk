...
describe('Besu.Backup', function () {
  this.timeout(100000)

  const backup = new Backup(config, 'besu')
  ...
  describe('Besu.Backup.getExportItems', () => {
  ...
  describe('Besu.Backup.getBackupItems', () => {
  ...
  describe('Besu.Backup.import', () => {
    it('should import a backup tarball', async () => {
      fs.rmSync(resolve(`${bdkPath}/bdk-besu-network`), { recursive: true })
      fs.mkdirSync(resolve(`${bdkPath}/bdk-besu-network`))
      await sleep(500)
      ...
      assert(fs.readdirSync(resolve(`${bdkPath}/bdk-besu-network`)).length > 0, 'No files found in bdk-besu-network folder')
    })
