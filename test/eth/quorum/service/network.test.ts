...
describe.skip('Quorum.Network.Service', function () {
  ...
  describe('Quorum.Network.add', () => {
    ...
    describe('Quorum.Network.addValidatorLocal', () => {
      it('should add a new validator locally', async () => {
        ...
        const newValidatorNum = await network.addValidatorLocal(NetworkType.QUORUM)
        ...
        const networkType = NetworkType.QUORUM

        const isValidator = await network.checkNode(networkType, `validator${validatorNumAfter - 1}`, 'istanbul.isValidator()')
        assert.strictEqual(isValidator, 'true')
      })

      it('should throw timeout Error', async () => {
        const stub = sinon.stub(network, 'quorumCommand' as keyof Network).resolves('false')
        await assert.rejects(async () => { await network.addValidatorLocal(NetworkType.QUORUM) }, TimeLimitError)
        stub.restore()
      })
    })
    ...
    describe('Quorum.Network.addMemberLocal', () => {
      it('should add a new member locally', async () => {
        ...
        const newMemberNum = await network.addMemberLocal(NetworkType.QUORUM)
      ...
      it('should throw timeout Error', async () => {
        ...
        await assert.rejects(async () => { await network.addMemberLocal(NetworkType.QUORUM) }, TimeLimitError)
        stub.restore()
      })
    })
    ...
    describe('Quorum.Network.joinNode', () => {
      ...
      it('should join a validator node successfully', async () => {
        ...
        await network.joinNode(NetworkType.QUORUM, joinNodeConfig)
        quorumStub.restore()
      ...
      it('should throw timeout Error when join validator', async () => {
        ...
        await assert.rejects(async () => { await network.joinNode(NetworkType.QUORUM, joinNodeConfig) }, TimeLimitError)
        quorumStub.restore()
      })
  ...
  describe('Quorum.Network.getNetworkInfo', () => {
    ...
    before(async () => {
      ...
      await network.create(networkCreateConfig)
      newValidatorNum = await network.addValidatorLocal(NetworkType.QUORUM)
      newMemberNum = await network.addMemberLocal(NetworkType.QUORUM)
    })
